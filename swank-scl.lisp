...
;;;
;;; Scieneer Common Lisp code for SLCLJ.
;;;
...
(defclass slclj-input-stream (ext:character-input-stream)
...
(defun make-slclj-input-stream (input-fn)
  (declare (function input-fn))
  (make-instance 'slclj-input-stream
...
(defmethod print-object ((s slclj-input-stream) stream)
  (print-unreadable-object (s stream :type t)))

;;; input-stream-p inherits from input-stream.
;;; output-stream-p inherits nil.

(defmethod ext:stream-listen ((stream slclj-input-stream))
...
(defmethod close ((stream slclj-input-stream) &key ((:abort abort) nil))
...
(defmethod ext:stream-clear-input ((stream slclj-input-stream))
...
(defmethod file-length ((stream slclj-input-stream))
  (let* ((input-buffer (slot-value stream 'buffer))
         (index (slot-value stream 'index))
         (input-length (length input-buffer))
         (available (- input-length index))
...
(defmethod ext:stream-file-position ((stream slclj-input-stream)
                                     &optional position)
...
(defmethod interactive-stream-p ((stream slclj-input-stream))
  (slot-value stream 'interactive))

;;; No 'file-string-length method.

(defmethod ext:stream-read-chars ((stream slclj-input-stream) buffer
...
(defclass slclj-output-stream (ext:character-output-stream)
...
(defun make-slclj-output-stream (output-fn)
  (declare (function output-fn))
  (make-instance 'slclj-output-stream
		 :in-buffer ""
		 :out-buffer ""
...
(defmethod print-object ((s slclj-output-stream) stream)
  (print-unreadable-object (s stream :type t)))
...
(defmethod close ((stream slclj-output-stream) &key ((:abort abort) nil))
  (when (ext:stream-open-p stream)
    (unless abort
...
(defmethod ext:stream-finish-output ((stream slclj-output-stream))
...
(defmethod ext:stream-force-output ((stream slclj-output-stream))
  (ext:stream-finish-output stream)
  nil)

(defmethod ext:stream-clear-output ((stream slclj-output-stream))
...
(defmethod ext:stream-line-length ((stream slclj-output-stream))
  80)

(defmethod ext:stream-line-column ((stream slclj-output-stream))
  (slot-value stream 'column))

(defmethod file-length ((stream slclj-output-stream))
  (slot-value stream 'position))

(defmethod ext:stream-file-position ((stream slclj-output-stream)
                                     &optional position)
...
(defmethod interactive-stream-p ((stream slclj-output-stream))
  (slot-value stream 'interactive))
...
;;;
(defmethod ext:stream-write-char ((stream slclj-output-stream) character)
...
;;; stream-write-chars
;;;
(defmethod ext:stream-write-chars ((stream slclj-output-stream)
...
(defimplementation make-output-stream (output-fn)
  (make-slclj-output-stream output-fn))

(defimplementation make-input-stream (input-fn)
  (make-slclj-input-stream input-fn))
...
(defun location-in-stream (code-location debug-source)
  ...
  (unless (debug-source-info-from-emacs-buffer-p debug-source)
    (error "The code is compiled from a non-SLCLJ stream."))
