...
;;; The `DEFSLCLJFUN' macro defines a function that Emacs can call via
;;; RPC.

(defmacro defslcljfun (name arglist &body rest)
...
;; Note: `with-connection' binds *pending-slclj-interrupts*.
(defvar *slclj-interrupts-enabled*)

(defmacro with-interrupts-enabled% (flag body)
  `(progn
     (check-slclj-interrupts)
     (multiple-value-prog1
         (let ((*slclj-interrupts-enabled* ,flag))
           ,@body)
       (check-slclj-interrupts))))

(defmacro with-slclj-interrupts (&body body)
  `(with-interrupts-enabled% t ,body))

(defmacro without-slclj-interrupts (&body body)
  `(with-interrupts-enabled% nil ,body))

(defun invoke-or-queue-interrupt (function)
  (log-event "invoke-or-queue-interrupt: ~a~%" function)
  (cond ((not (boundp '*slclj-interrupts-enabled*))
         (without-slclj-interrupts
           (funcall function)))
        (*slclj-interrupts-enabled*
         (log-event "interrupts-enabled~%")
         (funcall function))
        (t
         (setq *pending-slclj-interrupts*
               (nconc *pending-slclj-interrupts*
                      (list function)))
         (cond ((cdr *pending-slclj-interrupts*)
...
(defslcljfun simple-break (&optional (datum "Interrupt from Emacs") &rest args)
  (with-simple-restart (continue "Continue from break.")
    (invoke-slclj-debugger (coerce-to-condition datum args))))
...
if the file doesn't exist; otherwise the first line of the file."
  (with-open-file (in
                   (merge-pathnames (user-homedir-pathname) #p".slclj-secret")
  ...
  `(with-connection (,connection)
     (restart-case
         ...
         (let ((*sldb-quit-restart* (find-restart 'abort))
           ...
           ,@body)
       (abort (&optional v)
         :report "Return to SLCLJ's top level."
         (declare (ignore v))
         (force-user-output)
         ,k))))
...
(defun top-level-restart-p ()
  ...
  ;;
  ;; The caveat is that for slclj rex requests, we do not want to use
         ...
         (call-with-user-break-handler
          (lambda ()
            (invoke-or-queue-interrupt
             #'(lambda () (dispatch-interrupt-event connection))))
          (lambda ()
            (with-simple-restart (close-connection "Close SLCLJ connection")
              ;;(handle-requests connection)
       ...
       (with-top-level-restart (connection nil)
         (let* ((socket (connection.socket-io connection))
           ...
           (cond ((eq ready :interrupt)
                  (check-slclj-interrupts))
                 ((member socket ready)
                  ;; A Slclj request from Emacs is pending; make sure to
...
(defslcljfun create-repl (target)
...
(defslcljfun create-listener (remote)
  (let* ((pkg *package*)
...
(defvar *swank-wire-protocol-version* nil
  "The version of the swank/slclj communication protocol.")

(defslcljfun connection-info ()
...
(defslcljfun io-speed-test (&optional (n 1000) (m 1))
...
(defslcljfun toggle-debug-on-swank-error ()
...
(define-special *buffer-package*     
    "Package corresponding to slclj-buffer-package.  

EVAL-FOR-EMACS binds *buffer-package*.  Strings originating from a slclj
  ...
  (let (ok result)
    (unwind-protect
         (let ((*buffer-package* (guess-buffer-package buffer-package))
           ...
           (setq result (with-slclj-interrupts (eval form)))
...
(defslcljfun interactive-eval (string)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ interactive evaluation request.")
      (let ((values (multiple-value-list (eval (from-string string)))))
        (finish-output)
        (format-values-for-echo-area values)))))

(defslcljfun eval-and-grab-output (string)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ evaluation request.")
...
(defslcljfun interactive-eval-region (string)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ interactive evaluation request.")
      (format-values-for-echo-area (eval-region string)))))

(defslcljfun re-evaluate-defvar (form)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ evaluation request.")
...
(defslcljfun pprint-eval (string)
  (with-buffer-syntax ()
    (let* ((s (make-string-output-stream))
           (values 
...
(defslcljfun listener-eval (string)
...
(defun repl-eval (string)
  (clear-user-input)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ REPL evaluation request.")
      (track-package 
       (lambda ()
...
(defslcljfun ed-in-emacs (&optional what)
  "Edit WHAT in Emacs.
...
(defslcljfun inspect-in-emacs (what &key wait)
...
(defslcljfun value-for-editing (form)
...
(defslcljfun commit-edited-value (form value)
...
(defun invoke-slclj-debugger (condition)
...
(defun swank-debugger-hook (condition hook)
  ...
  (handler-case
      (call-with-debugger-hook #'swank-debugger-hook
                               (lambda () (invoke-slclj-debugger condition)))
...
(defslcljfun sldb-break-with-default-debugger (dont-unwind)
...
(defslcljfun backtrace (start end)
...
(defslcljfun debugger-info-for-emacs (start end)
...
Operation was KERNEL::DIVISION, operands (1 0).\"
  ...
  ((\"ABORT\" \"Return to Slclj toplevel.\")
...
(defslcljfun invoke-nth-restart (index)
  (invoke-restart-interactively (nth-restart index)))

(defslcljfun sldb-abort ()
  (invoke-restart (find 'abort *sldb-restarts* :key #'restart-name)))

(defslcljfun sldb-continue ()
  (continue))
...
(defslcljfun invoke-nth-restart-for-emacs (sldb-level n)
...
(defslcljfun eval-string-in-frame (string index)
  (values-to-string
   (eval-in-frame (wrap-sldb-vars (from-string string))
                  index)))

(defslcljfun pprint-eval-string-in-frame (string index)
  (swank-pprint
   (multiple-value-list 
    (eval-in-frame (wrap-sldb-vars (from-string string)) index))))

(defslcljfun frame-locals-and-catch-tags (index)
...
(defslcljfun compile-file-for-emacs (filename load-p &key options policy)
...
(defslcljfun compile-string-for-emacs (string buffer position filename policy)
...
(defslcljfun compile-multiple-strings-for-emacs (strings policy)
...
(defslcljfun compile-file-if-needed (filename loadp)
  (let ((pathname (filename-to-pathname filename)))
...
(defslcljfun load-file (filename)
  (to-string (load (filename-to-pathname filename))))


;;;;; swank-require

(defslcljfun swank-require (modules &optional filename)
...
(defslcljfun swank-macroexpand-1 (string)
  (apply-macro-expander #'macroexpand-1 string))

(defslcljfun swank-macroexpand (string)
  (apply-macro-expander #'macroexpand string))

(defslcljfun swank-macroexpand-all (string)
  (apply-macro-expander #'macroexpand-all string))

(defslcljfun swank-compiler-macroexpand-1 (string)
  (apply-macro-expander #'compiler-macroexpand-1 string))

(defslcljfun swank-compiler-macroexpand (string)
  (apply-macro-expander #'compiler-macroexpand string))

(defslcljfun swank-format-string-expand (string)
  (apply-macro-expander #'format-string-expand string))

(defslcljfun disassemble-symbol (name)
  (with-buffer-syntax ()
    (with-output-to-string (*standard-output*)
      (let ((*print-readably* nil))
...
(defslcljfun simple-completions (prefix package)
...
(defslcljfun operator-arglist (name package)
  (ignore-errors
...
(defslcljfun apropos-list-for-emacs  (name &optional external-only 
...
(defslcljfun list-all-package-names (&optional nicknames)
...
(defslcljfun swank-toggle-trace (spec-string)
...
(defslcljfun untrace-all ()
  (untrace))

(defslcljfun redirect-trace-output (target)
...
(defslcljfun undefine-function (fname-string)
...
(defslcljfun toggle-profile-fdefinition (fname-string)
  (let ((fname (from-string fname-string)))
...
(defslcljfun profile-by-substring (substring package)
  (let ((count 0))
...
(defslcljfun find-definition-for-thing (thing)
  (find-source-location thing))

(defslcljfun find-source-location-for-emacs (spec)
...
(defslcljfun find-definitions-for-emacs (name)
...
(defslcljfun init-inspector (string)
  (with-buffer-syntax ()
    (with-retry-restart (:msg "Retry SLCLJ inspection request.")
      (reset-inspector)
...
(defslcljfun inspector-nth-part (index)
  (aref (istate.parts *istate*) index))

(defslcljfun inspect-nth-part (index)
  (with-buffer-syntax ()
    (let ((*inspector-verbose* (istate.verbose *istate*)))
      (inspect-object (inspector-nth-part index)))))

(defslcljfun inspector-range (from to)
  (prepare-range *istate* from to))

(defslcljfun inspector-call-nth-action (index &rest args)
...
(defslcljfun quit-thread-browser ()
  (setq *thread-list* nil))

(defun nth-thread (index)
  (nth index *thread-list*))

(defslcljfun debug-nth-thread (index)
...
(defslcljfun kill-nth-thread (index)
  (kill-thread (nth-thread index)))

(defslcljfun start-swank-server-in-thread (index port-file-name)
...
(defslcljfun mop (type symbol-name)
...
(defun sync-features-to-emacs ()
  ...
  ;; FIXME: *slclj-features* should be connection-local
  (unless (eq *slclj-features* *features*)
    (setq *slclj-features* *features*)
    (send-to-emacs (list :new-features (features-for-emacs)))))

(defun features-for-emacs ()
  "Return `*slclj-features*' in a format suitable to send it to Emacs."
  *slclj-features*)
...
(defslcljfun update-indentation-information ()
