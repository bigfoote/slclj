;;; swank-mit-scheme.scm --- SLCLJ server for MIT Scheme
;;
...
(setq slclj-lisp-implementations
...
(setq slclj-find-buffer-package-function 'find-mit-scheme-package)
   ...
   -*- slclj-buffer-package: "(FOO)" -*- file variables.
...
(define (main-loop socket)
  (do () (#f)
    (with-simple-restart 
	'abort "Return to SLCLJ top-level."
	(lambda () 
...

;;;; Reading/Writing of SLCLJ packets

(define (read-packet in)
  "Read an S-expression from STREAM using the SLCLJ protocol."
