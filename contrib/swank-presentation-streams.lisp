...
;; This file contains a mechanism for printing to the slclj repl so
...
;;
;; We only do this if we know we are printing to a slclj stream,
;; checked with the method slclj-stream-p. Initially this checks for
;; the knows slclj streams looking at *connections*. In cmucl, sbcl, and
;; openmcl it also checks if it is a pretty-printing stream which
;; ultimately prints to a slclj stream.
;;
...
(let ((last-stream nil)
      (last-answer nil))
  (defun slclj-stream-p (stream)
    "Check if stream is one of the slclj streams, since if it isn't we
	  ...
	  (setq last-answer 
		(or #+openmcl 
		    (and (typep stream 'ccl::xp-stream) 
					;(slclj-stream-p (ccl::xp-base-stream (slot-value stream 'ccl::xp-structure)))
			 (slclj-stream-p (ccl::%svref (slot-value stream 'ccl::xp-structure) 1)))
		    #+cmu
		    (or (and (typep stream 'lisp::indenting-stream)
			     (slclj-stream-p (lisp::indenting-stream-stream stream)))
			(and (typep stream 'pretty-print::pretty-stream)
			     (fboundp 'pretty-print::enqueue-annotation)
			     (let ((slclj-stream-p
				    (slclj-stream-p (pretty-print::pretty-stream-target stream))))
			       (and ;; Printing through CMUCL pretty
				    ...
				    (not (eql slclj-stream-p :dedicated-output))
				    ;; If OK, return the return value
				    ;; we got from slclj-stream-p on
		    ...
		    (let ()
		      ...
		      (and (typep stream (find-symbol "PRETTY-STREAM" 'sb-pretty))
                           ...
                           (slclj-stream-p (sb-pretty::pretty-stream-target stream))))
		    #+allegro
		    (and (typep stream 'excl:xp-simple-stream)
			 (slclj-stream-p (excl::stream-output-handle stream)))
