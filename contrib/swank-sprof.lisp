...
(defslcljfun swank-sprof-get-call-graph (&key exclude-swank)
  (setf *call-graph* (sb-sprof:report :type nil))
  (serialize-call-graph :exclude-swank exclude-swank))

(defslcljfun swank-sprof-expand-node (index)
...
(defslcljfun swank-sprof-disassemble (index)
  (let* ((node (gethash index *number-nodes*))
...
(defslcljfun swank-sprof-source-location (index)
  (let* ((node (gethash index *number-nodes*))
         (debug-info (sb-sprof::node-debug-info node)))
...
(defslcljfun swank-sprof-start ()
  (sb-sprof:start-profiling))

(defslcljfun swank-sprof-stop ()
