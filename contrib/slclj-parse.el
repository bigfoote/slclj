;;; slclj-parse.el --- parsing of Common Lisp source code
;;
...
(defun slclj-parse-form-until (limit form-suffix)
  ...
  (let ((todo (list (point))) ; stack of positions
    ...
    (while (and (setq curpos (pop todo))
                (progn
                  ...
                  (ignore-errors (slclj-forward-cruft))
                  (< (point) limit)))
...
(defun slclj-compare-char-syntax (get-char-fn syntax &optional unescaped)
...
(defconst slclj-cursor-marker 'swank::%cursor-marker%)

(defun slclj-parse-form-upto-point (&optional max-levels)
  (save-restriction
    ...
    (save-excursion
      (let ((suffix (list slclj-cursor-marker)))
        ...
        (let ((pt (point)))
          ...
          (slclj-parse-form-until pt suffix))))))


;;;; Test cases

(defun slclj-check-buffer-form (result-form)
  (slclj-test-expect 
   (format "Buffer form correct in `%s' (at %d)" (buffer-string) (point))
   result-form
   (slclj-parse-form-upto-point 10)))

(def-slclj-test form-up-to-point.1
  ...
  (with-temp-buffer
    ...
    (slclj-check-buffer-form result-form)
...
(let ((byte-compile-warnings '()))
  (mapc #'byte-compile
        '(slclj-parse-form-upto-point
          slclj-parse-form-until
          slclj-compare-char-syntax
          )))