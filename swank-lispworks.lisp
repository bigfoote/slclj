...
;;;
;;; swank-lispworks.lisp --- LispWorks specific code for SLCLJ. 
;;;
...
(defclass slclj-env (env:environment) 
  ((debugger-hook :initarg :debugger-hoook)))

(defun slclj-env (hook io-bindings) 
  (make-instance 'slclj-env :name "SLCLJ Environment" 
                 :io-bindings io-bindings
                 :debugger-hoook hook))

(defmethod env-internals:environment-display-notifier 
    ((env slclj-env) &key restarts condition)
  (declare (ignore restarts condition))
  (funcall (swank-sym :swank-debugger-hook) condition *debugger-hook*)
  ;;  nil
  )

(defmethod env-internals:environment-display-debugger ((env slclj-env))
  *debug-io*)

(defmethod env-internals:confirm-p ((e slclj-env) &optional msg &rest args)
  (apply (swank-sym :y-or-n-p-in-emacs) msg args))

(defimplementation call-with-debugger-hook (hook fun)
  (let ((*debugger-hook* hook))
    (env:with-environment ((slclj-env hook '()))
      (funcall fun))))

(defimplementation install-debugger-globally (function)
  (setq *debugger-hook* function)
  (setf (env:environment) (slclj-env function '())))
...
(defimplementation initialize-multiprocessing (continuation)
  (cond ((not mp::*multiprocessing*)
         (push (list "Initialize SLCLJ" '() continuation) 
