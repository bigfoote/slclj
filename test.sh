...
# Run the SLCLJ test suite inside screen, saving the results to a file.
...
Usage: $name [-bsRTS] [-n <name>] <emacs> <lisp>"
  ...
  -T  no temp directory (use slclj in current directory)
...
slcljdir=$(dirname $name)
tmpdir=/tmp/slclj-test.$$
...
mkdir $tmpdir
if [ $use_temp_dir == true ] ; then 
    cp -r $slcljdir/*.{el,lisp} ChangeLog $tmpdir 
    # cp -r $slcljdir/contrib $tmpdir 
fi
...
cmd=($emacs -nw -q -no-site-file $batch_mode --no-site-file
       ...
       --eval "(slclj-batch-test \"$results\" $test_name $randomize)")

if [ "$use_screen" = "" ]; then
    "${cmd[@]}"
    echo $? > $statusfile
else 
    session=slclj-screen.$$
