     ...
     (let* ((f (constantly t))
       ...
       (unwind-protect
            (let ((*interrupt-queued-handler* (lambda () 
                                                (write-char #\! out))))
              (when (check-slclj-interrupts) (return :interrupt))
              (sys:serve-event))
...
(defimplementation make-output-stream (write-string)
  (make-slclj-output-stream write-string))

(defimplementation make-input-stream (read-string)
  (make-slclj-input-stream read-string))

(defstruct (slclj-output-stream
             (:include lisp::lisp-stream
                       (lisp::misc #'sos/misc)
                       (lisp::out #'sos/write-char)
                       (lisp::sout #'sos/write-string))
             (:conc-name sos.)
             (:print-function %print-slclj-output-stream)
             (:constructor make-slclj-output-stream (output-fn)))
  (output-fn nil :type function)
  (buffer (make-string 4000) :type string)
  (index 0 :type kernel:index)
  (column 0 :type kernel:index))

(defun %print-slclj-output-stream (s stream d)
  (declare (ignore d))
...
(defstruct (slclj-input-stream
             (:include string-stream
                       (lisp::in #'sis/in)
                       (lisp::misc #'sis/misc))
             (:conc-name sis.)
             (:print-function %print-slclj-output-stream)
             (:constructor make-slclj-input-stream (input-fn)))
