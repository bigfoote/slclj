...
;;;
;;; slclj-backend.lisp --- SLCLJ backend interface.
;;;
...
slclj-print-apropos in Emacs must know about them).
...
(defvar *pending-slclj-interrupts*)

(defun check-slclj-interrupts ()
...
Return a boolean indicating whether any interrupts was processed."
  (when (and (boundp '*pending-slclj-interrupts*)
             *pending-slclj-interrupts*)
    (funcall (pop *pending-slclj-interrupts*))
    t))
...
(defun wait-for-one-stream (stream timeout)
  (ecase timeout
    ((nil)
     (cond ((check-slclj-interrupts) :interrupt)
